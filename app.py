...
def load_data(file_path):
    ...
    for i in range(proc_header, len(df_sheet1)):
        if df_sheet1.iloc[i].isnull().all():
            proc_end = i
            break
    else:
        proc_end = len(df_sheet1)
    ...
    for i in range(crm_header, len(df_sheet1)):
        if df_sheet1.iloc[i].isnull().all():
            crm_end = i
            break
    else:
        crm_end = len(df_sheet1)
    ...
    # --- Proc. Center & CRM Summary (detail) ---
    ...
    for i in range(detail_header, len(df_sheet1)):
        if df_sheet1.iloc[i].isnull().all():
            detail_end = i
            break
    else:
        detail_end = len(df_sheet1)
    ...
    # --- Sheet2: Detailed transactions (header at row 4) ---
    try:
...
if not data_dict:
    st.error("No valid data loaded. Please check the errors above.")
    st.stop()
...
fig_ts = px.line(ts_data, x='Date', y='Deposit Positional', title='Total Deposits by Date', markers=True)
...
fig_center.add_trace(go.Bar(x=center_data['PROCESSING_CENTER'], y=center_data['Deposit Positional'],
                             name='Deposit Positional', marker_color='royalblue'))
...
st.dataframe(rm_display, use_container_width=True, hide_index=True,
             column_config={
...
if not scatter_data.empty:
    fig_scatter = px.scatter(scatter_data, x='Incremental', y='Achievment Percentage',
                             hover_data=['RM_NAME', 'Date'], color='Date',
...
if selected_rm:
    ...
    if trans_list:
        ...
        st.write(f"**{selected_rm}** – Total Deposits: {trans_combined['Deposit Positional'].sum():,.2f}, "
                 f"Incremental: {trans_combined['INCRIENTAL MOBILIZED'].sum():+,.2f}")
        st.dataframe(trans_combined[['Date', 'PROCESSING_CENTER', 'BRANCH_NAME', 'AC_DESC',
                                     'Baseline', 'Deposit Positional', 'INCRIENTAL MOBILIZED']],
...
if st.sidebar.button("Download Filtered Data (CSV)"):
    ...
    st.sidebar.download_button(label="Download CSV", data=csv,
